...
public class Test4Activity extends ComponentActivity {
    ...
    private final boolean[] logDebug = {true, false, false, false, false};
    private final boolean syncDebug = true;
    private final long[] SYNC_INTERVALS_SECONDS = {2,4,6,7,8,9,11,12,13,14};
//    private final long[] SYNC_INTERVALS_SECONDS = {1,5,10,30,45,60,75,90,120};
//    private final long[] SYNC_INTERVALS_SECONDS = {3,15,20,25};
    private long syncInterval;
    private int syncIntervalIndex = 0;
    private int samplingRate = 52;// 13, 26, 52, 104, 208, 416, 833, 1666
    private long recordingDuration = 1* 60 * 60 * 1000;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_test);
        syncLogger = new CsvLogger(this, "test4_sync_time.csv");
        syncLogger.logData("SamplingRate,RecordingDuration,SyncInterval,IMUCount,HeartRateCount,LocationCount,AccelerometerCount,GyroscopeCount");

        findViewById(R.id.start_button).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                syncIntervalIndex = 0;
                syncInterval = SYNC_INTERVALS_SECONDS[syncIntervalIndex] * 1000;
    ...
    private void startRecording() {
        if (isRecording) {
            ...
            Toast.makeText(Test4Activity.this, "Already Recording", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Log.d(TAG, "Start Recording, Sync Interval: " + SYNC_INTERVALS_SECONDS[syncIntervalIndex]);
        String fileName = "test4_" + SYNC_INTERVALS_SECONDS[syncIntervalIndex] + ".csv";
        csvLogger = new CsvLogger(Test4Activity.this, fileName);
        ...
        syncDuringRecording();
        Toast.makeText(Test4Activity.this, "Started Recording", Toast.LENGTH_SHORT).show();
    }
    ...
    private void stopRecording() {
        if (!isRecording) {
            ...
            Toast.makeText(Test4Activity.this, "Not Recording", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (syncIntervalIndex < SYNC_INTERVALS_SECONDS.length - 1) {
            syncIntervalIndex++;
            syncInterval = SYNC_INTERVALS_SECONDS[syncIntervalIndex] * 1000;
            startRecording(recordingDuration);
        } else {
            Toast.makeText(Test4Activity.this, "Finished Recording", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void subscribeImuData() {
        ...
        MdsManager mdsManager = new MdsManager(this, device);
    ...
    private void syncAfterRecording() {
        ...
        syncLogger.logData(samplingRate + "," + recordingDuration + "," + syncInterval + "," + TimeUtil.formatDuration(syncEndTime - syncStartTime) + "," + dataCounts[0] + "," + dataCounts[1] + "," + dataCounts[2] + "," + dataCounts[3] + "," + dataCounts[4]);
    }
