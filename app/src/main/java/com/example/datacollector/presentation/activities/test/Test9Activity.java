...
import com.example.datacollector.presentation.util.BinaryLogger;
import com.example.datacollector.presentation.util.CsvLogger;
import com.example.datacollector.presentation.util.JsonLogger;
import com.example.datacollector.presentation.util.Logger;
...
public class Test9Activity extends ComponentActivity {
    ...
    private final long[] RECORDING_DURATIONS_MIN = {60};
    private long syncInterval = 45 * 1000;
    private String fileName;
    private long recordingDuration;
    private int recordingDurationIndex;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_test);
        syncLogger = new CsvLogger(this, "test9_sync_time.csv");
        ...
        findViewById(R.id.start_button).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                recordingDurationIndex = 0;
                recordingDuration = RECORDING_DURATIONS_MIN[recordingDurationIndex] * 60 * 1000;
    ...
    private void startRecording() {
        if (isRecording) {
            ...
            Toast.makeText(Test9Activity.this, "Already Recording", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Log.d(TAG, "Start Recording, Recording Duration: " + RECORDING_DURATIONS_MIN[recordingDurationIndex]);
        ...
        syncDuringRecording();
        Toast.makeText(Test9Activity.this, "Started Recording", Toast.LENGTH_SHORT).show();
    }
    ...
    private void stopRecording() {
        if (!isRecording) {
            ...
            Toast.makeText(Test9Activity.this, "Not Recording", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (recordingDurationIndex < RECORDING_DURATIONS_MIN.length - 1) {
            recordingDurationIndex++;
            recordingDuration = RECORDING_DURATIONS_MIN[recordingDurationIndex] * 60 * 1000;
            startRecording(recordingDuration);
        } else {
            Toast.makeText(Test9Activity.this, "Recording Completed", Toast.LENGTH_SHORT).show();
        }
    }


    public void initialLogger(){
//        fileName = "test9_" + RECORDING_DURATIONS_MIN[recordingDurationIndex] + ".csv";
//        logger= new CsvLogger(Test9Activity.this, fileName);
//        logger.logData("date,timestamp,type,data");

//        fileName = "test9_" + RECORDING_DURATIONS_MIN[recordingDurationIndex] + ".json";
//        logger= new JsonLogger(Test9Activity.this, fileName);

        fileName = "test9_" + RECORDING_DURATIONS_MIN[recordingDurationIndex] + ".bin";
         logger= new BinaryLogger(Test9Activity.this, fileName);
    }
    ...
    public void subscribeImuData() {
        ...
        mdsSubscription = mdsManager.subscribeImu(samplingRate, new MdsNotificationListener() {
            ...
            public void onNotification(String data) {
                ...
                Log.d(TAG,"Received IMU Data: " + data);
    ...
    private void syncAfterRecording() {
        ...
        syncLogger.logData(samplingRate + "," + TimeUtil.formatDuration(syncStartTime - recordingStartTime) + "," + TimeUtil.formatDuration(syncEndTime - syncStartTime) + "," + dataCounts[0] + "," + dataCounts[1] + "," + dataCounts[2] + "," + dataCounts[3] + "," + dataCounts[4]);
    }


    private void sync(PriorityQueue<SensorData> queue) {
        while (!queue.isEmpty()) {
            SensorData sensorData = queue.poll();
            String date = TimeUtil.formatDate(sensorData.timestamp);
            logger.logData(date, sensorData.timestamp, sensorData.dataType, sensorData.data);
