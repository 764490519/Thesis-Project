...
public class Test7Activity extends ComponentActivity {
    ...
    private final boolean[] logDebug = {true, true, true, true, true};
...
//    private final long RECORDING_DURATION = 1 * 60 * 60 * 1000;//
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        syncLogger = new CsvLogger(this, "test7_time_sync.csv");
    ...
    private void startRecording() {
        if (isRecording) {
            ...
            Toast.makeText(Test7Activity.this, "Already Recording", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        String fileName = "test7_" + samplingRate + ".csv";
        csvLogger = new CsvLogger(Test7Activity.this, fileName);
        csvLogger.logData("date,timestamp,type,x,y,z");

        subscribeImuData();
        subscribeAccelerometerData();
        subscribeGyroscopeData();
//        syncDuringRecording();
        Toast.makeText(Test7Activity.this, "Started Recording", Toast.LENGTH_SHORT).show();
    }
    ...
    private void stopRecording() {
        if (!isRecording) {
            ...
            Toast.makeText(Test7Activity.this, "Not Recording", Toast.LENGTH_SHORT).show();
            return;
        }
...
//                long baseTimestamp = imuModel.getBody().getTimestamp() + mdsManager.getStartTime();
                ...
                for (int i = 0; i < imuModel.getBody().getArrayAcc().length; i++) {
                    ...
                    double gyroX = imuModel.getBody().getArrayGyro()[i].getX();
                    double gyroY = imuModel.getBody().getArrayGyro()[i].getY();
                    double gyroZ = imuModel.getBody().getArrayGyro()[i].getZ();

                    sensorDataPriorityQueue.add(new SensorData(timestamp, "IMU_ACC",String.format(Locale.getDefault(), "%.6f,%.6f,%.6f", linearAccX, linearAccY, linearAccZ)));
                    sensorDataPriorityQueue.add(new SensorData(timestamp, "IMU_GYRO",String.format(Locale.getDefault(), "%.6f,%.6f,%.6f", gyroX, gyroY, gyroZ)));
    ...
    private void subscribeAccelerometerData() {
        accelerometerReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                sensorDataPriorityQueue.add(new SensorData(accelerometerModel.timestamp, "Accelerometer",  + accelerometerModel.x + "," + accelerometerModel.y + "," + accelerometerModel.z));
                if (logDebug[3]) {
    ...
    private void subscribeGyroscopeData() {
        gyroscopeReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                sensorDataPriorityQueue.add(new SensorData(gyroscopeModel.timestamp, "Gyroscope", gyroscopeModel.x + "," + gyroscopeModel.y + "," + gyroscopeModel.z));
                if (logDebug[4]) {
